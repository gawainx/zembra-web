...
- R055 `3bdb970` 完成 Supabase 模式的紧凑活动热力图：侧栏根据实际宽度与主题化的 16px 方块、4px 间距动态计算列数，再以列数乘 7 显式传入 store 和 client，Supabase 聚合随参数生成连续日期；移除可见日期和首末标签，保留 hover 与无障碍信息。20 个测试文件、137 项测试及两种生产构建均通过，等待用户验收。

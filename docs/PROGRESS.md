...
- R028 `6d1c67f` 以前向替换废弃 Supabase workspace 环境变量：Magic Link 使用用户输入邮箱，认证回跳后通过 schema v0.6.1 的 RLS 查询可访问 workspace 名称，用户选择后才激活业务 Client；完整测试与生产构建通过。`00d28a0` 后补齐 Magic Link 发送反馈，按钮依次显示“发送”“正在发送”“发送成功”，等待期间保持强调色并阻止重复请求，126 项测试与生产构建通过。

...
- R058 `eb9d449` 完成 Supabase Direct 空 Tag 子树删除：聚合数为 0 的 Tag 悬停或聚焦时显示垃圾桶，确认句为“确认删除 #路径？”，父 Tag 按深度倒序连同空后代删除；store 乐观移除、失败回滚并清空受影响筛选。21 个测试文件、140 项测试及两种生产构建通过，等待用户验收。

...
- [进行中] 将 `NoteCard` 展示态 tag 更新为与编辑器内 tag 一致的强调蓝实心胶囊。
- [待开始] 验证：运行笔记卡片、编辑器和首页测试，确认 tag 文本、编辑与筛选行为保持不变。

## Stage 3：收尾验证

- [待开始] 复核明亮和暗色主题下的首页、侧栏、设置页与弹窗共享 token。
- [待开始] 更新本计划的任务状态并运行完整测试与生产构建。

...
`WorkspaceSwitcher` 在提供改名回调时才展示修改图标，因此 Backend 模式保持现状。点击图标将名称替换为自动聚焦且全选的输入框，编辑态的修改图标替换为确认勾选；勾选、Enter 与失焦共用同一提交逻辑。空白名称停留在输入态，不发起请求。SupabaseEntry 保持 workspace 列表状态，并在更新成功后替换对应名称，让 context、下拉展示和浏览器标题由既有数据流同步刷新。

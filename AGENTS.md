...
- 交互强调色使用 Bonofix 风格的蓝色及其语义 token；链接、聚焦、选中态和信息性强调共享这一色系。错误、警告和成功状态继续使用各自的语义 token，不得挪用强调蓝表达状态。
...
- 主题调整必须同时覆盖明亮和暗色模式，并至少检查首页写作区、侧栏、笔记卡片、编辑器、设置页和弹窗，避免局部组件沿用旧的青绿色或冷白风格。

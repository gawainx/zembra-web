...
- `docs/PROGRESS.md` 项目进度追踪文件，包含两个章节。`项目流程记录` 章节记录项目计划与阶段推进状态，针对每个需求，记录需求编号、git HEAD id（保留前八位），然后使用连续的、不超过200字的简体中文描述本次开发实现的计划以及实现情况，每个需求只允许记录一个条目，如果存在该条目，则在已有内容基础上追加。该文件只允许在文档末尾写入，禁止擅自修改文档结构、在文档开头或者中间位置写入任何内容
